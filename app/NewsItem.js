...
export default class NewsItem extends React.Component {
	...
	getTitle() {
		return (
			<div className="newsItem_title">
				<a className="newsItem_titleLink" href={this.props.item.url ? this.props.item.url : 'https://news.ycombinator.com/item?id=' + this.props.item.id}>{this.props.item.title}</a>
				{
	...
	getSubtext() { // 分数, 作者, 时间, 评论数
		return (
			<div className="newsItem_subtext">
	...
	render() {
		return (
			...
			{this.getRank()}
			{this.getVote()}
				<div className="newsItem_itemText">
					{this.getTitle()}
					{this.getSubtext()}
				</div>
			</div>
